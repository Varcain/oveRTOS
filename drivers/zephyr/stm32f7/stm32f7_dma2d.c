 ...
 *
 * STM32F746 DMA2D (Chrom-ART) driver — the Zephyr ove_hal_dma2d backend. Same
 * register sequence as the other engines, Zephyr primitives: raw register pokes
 * (Zephyr ships no DMA2D driver), sys_cache_data_{flush,invd}_range for coherency
 * (the M7 D-cache runs ON and the guest SDRAM is a coherent cacheable region), and
 * a raw RCC clock enable (nothing else brings DMA2DEN up on Zephyr). The
...
#define D2_BASE 0x4002b000u
#define D2(off) (*(volatile uint32_t *)(D2_BASE + (off)))
#define D2_CR 0x00u
#define D2_ISR 0x04u
...
static void cache_plane(uintptr_t addr, uint32_t w, uint32_t h, uint32_t off, uint32_t cf, int inv)
{
	...
	size_t len = (size_t)((((addr + span - 1u) & ~(uintptr_t)31u) + 32u) - start);
	if (inv)
		sys_cache_data_invd_range((void *)start, len);
	else
		sys_cache_data_flush_range((void *)start, len);
}
