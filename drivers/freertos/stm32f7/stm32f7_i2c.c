		      ...
		      uint32_t timeout_ms)
{
	...
	ret = HAL_I2C_Master_Transmit(&i2c->hal_handle,
				      (uint16_t)(addr << 1),
	...
	default:          return OVE_ERR_BUS_NACK;
	}
}
		     ...
		     void *buf, size_t len, uint32_t timeout_ms)
{
	...
	ret = HAL_I2C_Master_Receive(&i2c->hal_handle,
				     (uint16_t)(addr << 1),
				     buf, (uint16_t)len,
				     timeout_ms);
	switch (ret) {
			   ...
			   uint32_t timeout_ms)
{
	...
	if (tx_len <= 2) {
		...
		ret = HAL_I2C_Mem_Read(&i2c->hal_handle,
				       (uint16_t)(addr << 1),
				       mem_addr, mem_size,
				       rx, (uint16_t)rx_len,
				       timeout_ms);
		...
		ret = HAL_I2C_Master_Sequential_Transmit_IT(
			&i2c->hal_handle, (uint16_t)(addr << 1),
