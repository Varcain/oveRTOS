...
{
	switch (instance) {
	case 0: return DEVICE_DT_GET_OR_NULL(DT_NODELABEL(spi1));
	case 1: return DEVICE_DT_GET_OR_NULL(DT_NODELABEL(spi2));
	case 2: return DEVICE_DT_GET_OR_NULL(DT_NODELABEL(spi3));
	case 3: return DEVICE_DT_GET_OR_NULL(DT_NODELABEL(spi4));
	default: return NULL;
	}
}
			 ...
			 size_t len, uint32_t timeout_ms)
{
	...
	switch (spi->mode) {
	case OVE_SPI_MODE_1: operation |= SPI_MODE_CPHA;  break;
	case OVE_SPI_MODE_2: operation |= SPI_MODE_CPOL;  break;
	case OVE_SPI_MODE_3: operation |= SPI_MODE_CPOL | SPI_MODE_CPHA; break;
	default: break;
	}
	...
	struct spi_buf_set tx_set = { .buffers = tx ? &tx_buf : NULL,
				      .count   = tx ? 1 : 0 };
	struct spi_buf_set rx_set = { .buffers = rx ? &rx_buf : NULL,
				      .count   = rx ? 1 : 0 };
