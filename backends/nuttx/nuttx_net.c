...
static void sockaddr_to_nuttx(const ove_sockaddr_t *ove,
...
static void nuttx_to_sockaddr(const struct sockaddr_in *sin,
			      ove_sockaddr_t *ove)
{
...
int ove_netif_up(ove_netif_t netif, const ove_netif_config_t *cfg)
{
	...
	if (cfg->dns.addr[0] | cfg->dns.addr[1] |
		...
		dns_add_nameserver((struct sockaddr *)&dns_addr,
				   sizeof(dns_addr));
	}
			...
			uint32_t timeout_ms)
{
	...
	if (n == 0) return OVE_ERR_NET_CLOSED;
	if (received) *received = (size_t)n;
	if (src) nuttx_to_sockaddr(&sin, src);
	return OVE_OK;
}
