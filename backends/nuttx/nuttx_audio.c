...
static void *audio_thread_fn(void *arg)
{
    ...
    ret = ioctl(sc->record_fd, AUDIOIOC_GETBUFFERINFO,
    ...
    ret = ioctl(sc->play_fd, AUDIOIOC_GETBUFFERINFO,
    ...
    for (i = 0; i < rec_nbuffers; i++)
      {
        ...
        ret = ioctl(sc->record_fd, AUDIOIOC_ALLOCBUFFER,
    ...
    for (i = 0; i < play_nbuffers; i++)
      {
        ...
        ret = ioctl(sc->play_fd, AUDIOIOC_ALLOCBUFFER,
                    (unsigned long)&buf_desc);
        if (ret != sizeof(buf_desc))
          {
            goto err_out;
                                 ...
                                 struct ove_audio_buf *out)
{
    ...
    unsigned int bytes = frames * out->fmt->channels *
...
static int nuttx_sink_start(void *ctx)
{
    ...
    sc->audio_mq = mq_open("/" CONFIG_OVE_APP_NAME "_audio",
                            O_RDWR | O_CREAT, 0644, &mq_attr_s);
...
static int nuttx_sink_process(void *ctx, const struct ove_audio_buf *in,
                               struct ove_audio_buf *out)
{
    (void)out;
    struct nuttx_sink_ctx *sc = (struct nuttx_sink_ctx *)ctx;
    ...
    unsigned int bytes = frames * in->fmt->channels *
                            ...
                            const char *name)
{
    ...
    int idx = ove_audio_graph_add_node(g, &nuttx_source_ops, ctx, name,
                          ...
                          const char *name)
{
    ...
    int idx = ove_audio_graph_add_node(g, &nuttx_sink_ops, ctx, name,
                                       OVE_AUDIO_NODE_SINK);
    return idx;
}
