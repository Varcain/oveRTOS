...
/* RX polling thread */
static void *nuttx_uart_rx_thread(void *arg)
{
...
int ove_hal_uart_open(ove_uart_t uart, const struct ove_uart_cfg *cfg)
{
	...
	snprintf(path, sizeof(path), "/dev/ttyS%u", cfg->instance);
...
int ove_hal_uart_rx_enable(ove_uart_t uart)
{
	...
	uart->running = 1;
	if (pthread_create(&uart->rx_thread, NULL, nuttx_uart_rx_thread, uart) != 0) {
		uart->running = 0;
