...
static int mqtt_recv(struct ove_mqtt_client *c, void *buf, size_t len,
...
static void dispatch_publish(struct ove_mqtt_client *c,
 ...
 */
static int mqtt_wait_for(struct ove_mqtt_client *c, uint8_t expected_type,
			 uint32_t timeout_ms)
{
	for (int attempts = 0; attempts < 10; attempts++) {
		size_t got = 0;
		int ret = mqtt_recv(c, c->rx_buf, c->rx_size, &got, timeout_ms);
		if (ret == OVE_ERR_TIMEOUT) continue; /* try again */
		...
		size_t offset = 0;
		while (offset < got) {
			size_t pkt_len = mqtt_packet_len(c->rx_buf + offset,
							 got - offset);
			if (pkt_len == 0) break;

			uint8_t pkt_type = c->rx_buf[offset] & 0xF0;
			if (pkt_type == expected_type) return OVE_OK;
...
void ove_mqtt_client_deinit(ove_mqtt_client_t client)
{
	if (client) {
		...
		if (client->rx_buf) OVE_BACKEND_FREE(client->rx_buf);
		if (client->tx_buf) OVE_BACKEND_FREE(client->tx_buf);
#endif
		     ...
		     const ove_mqtt_config_t *cfg)
{
	...
	if (!c->rx_buf || !c->tx_buf) return OVE_ERR_NO_MEMORY;
#endif
	...
	ret = ove_socket_open(&c->sock, &c->sock_storage,
	...
	*p++ = flags;
	put_u16(p, c->keep_alive_s); p += 2;

	/* Client ID */
	put_u16(p, (uint16_t)cid_len); p += 2;
	memcpy(p, cfg->client_id, cid_len); p += cid_len;

	/* Username */
	if (cfg->username) {
		put_u16(p, (uint16_t)usr_len); p += 2;
		memcpy(p, cfg->username, usr_len); p += usr_len;
	}
	/* Password */
	if (cfg->password) {
		put_u16(p, (uint16_t)pwd_len); p += 2;
		memcpy(p, cfg->password, pwd_len); p += pwd_len;
	}

	ret = mqtt_send(c, c->tx_buf, (size_t)(p - c->tx_buf));
	if (ret != OVE_OK) goto fail;
		     ...
		     ove_mqtt_qos_t qos)
{
	...
	if (qos == OVE_MQTT_QOS1) remaining += 2; /* packet ID */

	uint8_t *p = c->tx_buf;
	*p = MQTT_PUBLISH;
	if (qos == OVE_MQTT_QOS1) *p |= 0x02;
	p++;
	p += encode_remaining_length(p, remaining);

	put_u16(p, (uint16_t)topic_len); p += 2;
	memcpy(p, topic, topic_len); p += topic_len;

	if (qos == OVE_MQTT_QOS1) {
		put_u16(p, c->pkt_id++); p += 2;
		if (c->pkt_id == 0) c->pkt_id = 1;
	}
...
{
	if (!client || !client->connected) return OVE_ERR_INVALID_PARAM;
	struct ove_mqtt_client *c = client;
