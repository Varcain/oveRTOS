...
static int converter_configure(void *ctx, const struct ove_audio_fmt *in,
...
static int converter_process(void *ctx, const struct ove_audio_buf *in,
                             struct ove_audio_buf *out)
{
    ...
    if (src_fmt == dst_fmt) {
        memcpy(out->data, in->data,
               total * ove_audio_sample_size(src_fmt));
        return OVE_OK;
    }
...
static void converter_destroy(void *ctx) { free(ctx); }
                             ...
                             const char *name)
{
    ...
    int idx = ove_audio_graph_add_node(g, &converter_ops, ctx, name,
...
static int channel_map_process(void *ctx, const struct ove_audio_buf *in,
                               struct ove_audio_buf *out)
{
    struct channel_map_ctx *cm = (struct channel_map_ctx *)ctx;
...
static void channel_map_destroy(void *ctx) { free(ctx); }
...
int ove_audio_node_channel_map(struct ove_audio_graph *g,
                               ...
                               const char *name)
{
    if (!map || map->out_channels == 0 ||
        map->out_channels > OVE_AUDIO_MAX_CHANNELS)
        return OVE_ERR_INVALID_PARAM;
    struct channel_map_ctx *ctx = calloc(1, sizeof(*ctx));
    if (!ctx)
        return OVE_ERR_NO_MEMORY;
    ctx->map = *map;
    int idx = ove_audio_graph_add_node(g, &channel_map_ops, ctx, name,
                                       OVE_AUDIO_NODE_PROCESSOR);
    if (idx < 0)
        free(ctx);
    return idx;
...
static int gain_configure(void *ctx, const struct ove_audio_fmt *in,
                          struct ove_audio_fmt *out)
{
    (void)ctx;
    *out = *in;
...
static int gain_process(void *ctx, const struct ove_audio_buf *in,
                        struct ove_audio_buf *out)
{
...
static void gain_destroy(void *ctx) { free(ctx); }
...
int ove_audio_node_gain(struct ove_audio_graph *g, float gain_db,
                        const char *name)
{
...
static int tap_configure(void *ctx, const struct ove_audio_fmt *in,
                         struct ove_audio_fmt *out)
{
    (void)ctx;
    *out = *in;
...
static int tap_process(void *ctx, const struct ove_audio_buf *in,
                       struct ove_audio_buf *out)
{
...
static void tap_destroy(void *ctx) { free(ctx); }
...
int ove_audio_node_tap(struct ove_audio_graph *g, ove_audio_tap_fn fn,
                       void *user_data, const char *name)
{
    ...
    int idx = ove_audio_graph_add_node(g, &tap_ops, ctx, name,
                                       OVE_AUDIO_NODE_PROCESSOR);
    if (idx < 0)
        free(ctx);
    return idx;
