...
err_t ethernetif_init(struct netif *netif) { (void)netif; return ERR_IF; }
...
static void sockaddr_to_lwip(const ove_sockaddr_t *ove,
...
static void lwip_to_sockaddr(const struct sockaddr_in *sin,
			     ove_sockaddr_t *ove)
{
...
int ove_netif_up(ove_netif_t netif, const ove_netif_config_t *cfg)
{
	...
	if (cfg && !cfg->use_dhcp) {
		...
		IP4_ADDR(&ipaddr,
			 cfg->static_ip.addr[0], cfg->static_ip.addr[1],
			 cfg->static_ip.addr[2], cfg->static_ip.addr[3]);
		IP4_ADDR(&netmask,
			 cfg->netmask.addr[0], cfg->netmask.addr[1],
			 cfg->netmask.addr[2], cfg->netmask.addr[3]);
		IP4_ADDR(&gw,
			 cfg->gateway.addr[0], cfg->gateway.addr[1],
			 cfg->gateway.addr[2], cfg->gateway.addr[3]);
	} else {
	...
	struct netif *nif = netif_add(&s_netif, &ipaddr, &netmask, &gw,
	...
	if (cfg && (cfg->dns.addr[0] | cfg->dns.addr[1] |
		    cfg->dns.addr[2] | cfg->dns.addr[3])) {
		ip_addr_t dns_addr;
		IP4_ADDR(ip_2_ip4(&dns_addr),
	...
	xTaskCreateStatic(eth_rx_task, "eth_rx", 1024,
			  NULL, 4, s_eth_rx_stack, &s_eth_rx_tcb);
#else
		    ...
		    ove_af_t af, ove_sock_type_t type)
{
	...
	if (fd < 0) return lwip_errno_to_ove(errno);
	s->fd = fd;
	*sock = s;
	return OVE_OK;
}
...
{
	if (sock) {
		if (sock->fd >= 0) lwip_close(sock->fd);
		    ...
		    size_t *sent)
{
	...
	if (n < 0) return lwip_errno_to_ove(errno);
		    ...
		    size_t *received, uint32_t timeout_ms)
{
	...
	if (!ove_timeout_is_forever(timeout_ms)) {
		...
		lwip_setsockopt(sock->fd, SOL_SOCKET, SO_RCVTIMEO,
				&tv, sizeof(tv));
	}
		      ...
		      size_t *sent, const ove_sockaddr_t *dest)
{
	...
	ssize_t n = lwip_sendto(sock->fd, data, len, 0,
			...
			uint32_t timeout_ms)
{
	...
	ssize_t n = lwip_recvfrom(sock->fd, buf, len, 0,
	...
	if (n == 0) return OVE_ERR_NET_CLOSED;
	if (received) *received = (size_t)n;
	if (src) lwip_to_sockaddr(&sin, src);
	return OVE_OK;
}
