...
impl<'a> Request<'a> {
    ...
    pub fn body(&self) -> &[u8] {
        ...
        // SAFETY: null/zero-len handled above; `p` points to `len` bytes of
        // request body owned by the C request object, valid for `&self` (the
        // handler borrow that produced this `Request`).
