...
static void build_startable_graph(struct ove_audio_graph *g, int *node_ids, int *ctxs)
{
	graph_init(g, 256);
	ctxs[0] = 0;
	ctxs[1] = 1;
	ctxs[2] = 2;
	node_ids[0] = ove_audio_graph_add_node(g, &mock_startable_source_ops, &ctxs[0], "src",
					       OVE_AUDIO_NODE_SOURCE);
	node_ids[1] = ove_audio_graph_add_node(g, &mock_startable_proc_ops, &ctxs[1], "proc",
					       OVE_AUDIO_NODE_PROCESSOR);
	node_ids[2] = ove_audio_graph_add_node(g, &mock_startable_sink_ops, &ctxs[2], "sink",
					       OVE_AUDIO_NODE_SINK);
...
static void test_graph_add_node(void **state)
{
	...
	int idx =
		ove_audio_graph_add_node(&g, &mock_source_ops, NULL, "src", OVE_AUDIO_NODE_SOURCE);
	assert_int_equal(idx, 0);
	assert_int_equal(g.node_count, 1);

	idx = ove_audio_graph_add_node(&g, &mock_proc_ops, NULL, "proc", OVE_AUDIO_NODE_PROCESSOR);
...
static void test_graph_connect(void **state)
{
	...
	int src =
		ove_audio_graph_add_node(&g, &mock_source_ops, NULL, "src", OVE_AUDIO_NODE_SOURCE);
	int sink = ove_audio_graph_add_node(&g, &mock_sink_ops, NULL, "sink", OVE_AUDIO_NODE_SINK);
...
static void test_graph_connect_sink_output_rejected(void **state)
{
	...
	assert_int_equal(ove_audio_graph_connect(&g, sink, proc), OVE_ERR_INVALID_PARAM);
...
static void test_graph_connect_source_input_rejected(void **state)
{
	...
	assert_int_equal(ove_audio_graph_connect(&g, proc, src), OVE_ERR_INVALID_PARAM);
...
static void test_graph_build_cycle_rejected(void **state)
{
	...
	int a = ove_audio_graph_add_node(&g, &mock_proc_ops, NULL, "a", OVE_AUDIO_NODE_PROCESSOR);
	int b = ove_audio_graph_add_node(&g, &mock_proc_ops, NULL, "b", OVE_AUDIO_NODE_PROCESSOR);
...
static int doubler_process(void *ctx, const struct ove_audio_buf *in, struct ove_audio_buf *out)
{
	(void)ctx;
...
static int failing_process(void *ctx, const struct ove_audio_buf *in, struct ove_audio_buf *out)
{
	(void)ctx;
	(void)in;
	(void)out;
...
static void test_channel_map_stereo_to_mono(void **state)
{
	...
	struct ove_audio_channel_map map = {
		.out_channels = 1, .map = {1}, /* out ch0 = in ch1 (right) */
	};
...
static void test_graph_gain_dataflow(void **state)
{
	...
	int src = ove_audio_graph_add_node(&g, &dataflow_source_ops, NULL, "src",
					   OVE_AUDIO_NODE_SOURCE);
	int gain = ove_audio_node_gain(&g, -6.0f, "gain");

	int16_t captured[8] = {0};
	struct verify_sink_ctx vs = {.captured = captured};
	int sink = ove_audio_graph_add_node(&g, &verify_sink_ops, &vs, "sink", OVE_AUDIO_NODE_SINK);
...
static void test_graph_connect_after_build_fails(void **state)
{
	...
	assert_int_equal(ove_audio_graph_connect(&g, ids[0], ids[2]), OVE_ERR_NOT_SUPPORTED);
...
static void test_graph_max_edges(void **state)
{
	...
	int overflow =
		ove_audio_graph_add_node(&g, &mock_sink_ops, NULL, "xsink", OVE_AUDIO_NODE_SINK);
	assert_int_equal(overflow, OVE_ERR_QUEUE_FULL);
...
static void test_graph_connect_self_loop(void **state)
{
	...
	ove_audio_graph_add_node(&g, &mock_proc_ops, NULL, "proc", OVE_AUDIO_NODE_PROCESSOR);
...
static void test_graph_process_multiple_cycles(void **state)
{
	...
	int sink =
		ove_audio_graph_add_node(&g, &counting_sink_ops, &cs, "sink", OVE_AUDIO_NODE_SINK);
...
static int f32_source_process(void *ctx, const struct ove_audio_buf *in, struct ove_audio_buf *out)
{
	(void)ctx;
	(void)in;
...
static int s16_sink_configure(void *ctx, const struct ove_audio_fmt *in, struct ove_audio_fmt *out)
{
	(void)ctx;
	(void)out;
...
static int s16_sink_process(void *ctx, const struct ove_audio_buf *in, struct ove_audio_buf *out)
{
	(void)out;
...
static void test_converter_f32_to_s16(void **state)
{
	...
	int sink =
		ove_audio_graph_add_node(&g, &s16_sink_ops, &ss, "s16-sink", OVE_AUDIO_NODE_SINK);
...
static void test_channel_map_stereo_to_mono_node(void **state)
{
	...
	int src = ove_audio_graph_add_node(&g, &stereo_source_4f_ops, NULL, "stereo-4f",
					   OVE_AUDIO_NODE_SOURCE);

	/* Extract left channel (index 0) to mono */
	struct ove_audio_channel_map map = {
		.out_channels = 1, .map = {0}, /* out ch0 = in ch0 (left) */
	};
	int mapper = ove_audio_node_channel_map(&g, &map, "l-to-mono");

	int16_t captured[4] = {0};
	struct mono_capture_ctx mc = {.captured = captured};
	int sink = ove_audio_graph_add_node(&g, &mono_capture_ops, &mc, "mono-cap",
					    OVE_AUDIO_NODE_SINK);
