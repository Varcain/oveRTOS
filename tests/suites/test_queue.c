...
static void test_queue_producer_consumer(void **state)
{
    ...
    ove_test_thread_run(&th, &s_th_storage, "consumer", consumer_thread, q,
        s_th_stack, 4096);
...
static void test_queue_pair_item_size(void **state)
{
    ...
    ove_queue_t q = NULL;
    int rc = ove_test_queue_create(&q, &s_q_storage_pair, s_q_buf_pair,
                                sizeof(struct { int a; int b; }), 8);
    assert_int_equal(rc, OVE_OK);
    assert_non_null(q);

    struct { int a; int b; } item = { .a = 100, .b = 200 };
    assert_int_equal(ove_queue_send(q, &item, 0), OVE_OK);

    struct { int a; int b; } out = { 0 };
    assert_int_equal(ove_queue_receive(q, &out, 0), OVE_OK);
