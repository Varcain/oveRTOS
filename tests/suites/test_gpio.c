...
static void test_gpio_irq_register(void **state)
{
    ...
    int rc = ove_gpio_irq_register(0, 0, OVE_GPIO_IRQ_RISING,
...
static void test_gpio_irq_enable_disable(void **state)
{
    (void)state;
    ove_board_init();
    ove_gpio_irq_register(0, 0, OVE_GPIO_IRQ_RISING,
                              gpio_irq_handler, NULL);

    int rc = ove_gpio_irq_enable(0, 0);
    assert_int_equal(rc, OVE_OK);

    rc = ove_gpio_irq_disable(0, 0);
    assert_int_equal(rc, OVE_OK);
}
