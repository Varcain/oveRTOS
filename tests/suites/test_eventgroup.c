...
static void test_eg_wait_all(void **state)
{
    ...
    int rc = ove_eventgroup_wait_bits(eg, BIT_0 | BIT_1,
                                          OVE_EG_WAIT_ALL, 100, &actual);
    assert_int_equal(rc, OVE_OK);
...
static void test_eg_wait_timeout(void **state)
{
    ...
    ove_eventbits_t actual = 0;
    int rc = ove_eventgroup_wait_bits(eg, BIT_0,
                                          OVE_EG_WAIT_ALL, 10, &actual);
...
static void test_eg_cross_thread(void **state)
{
    ...
    ove_test_thread_run(&th, &s_th_storage, "setter", setter_thread, &ctx,
        s_th_stack, 4096);

    ove_eventbits_t actual = 0;
    int rc = ove_eventgroup_wait_bits(eg, BIT_0 | BIT_1,
                                          OVE_EG_WAIT_ALL, 500, &actual);
    assert_int_equal(rc, OVE_OK);
    assert_true((actual & (BIT_0 | BIT_1)) == (BIT_0 | BIT_1));
