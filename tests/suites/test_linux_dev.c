 ...
 * and drive ove_lnx_syscall() (no hardware SVC, no run loop) to check the
...
static long mock_open(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, int flags)
{
	(void)d;
	(void)o;
	(void)flags;
...
static long mock_release(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o)
{
	(void)d;
	(void)o;
	g_mock_released++;
...
static long mock_read(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p,
		      void *buf, size_t len)
{
	(void)d;
	(void)o;
	(void)p;
	if (g_mock_block)
		return -OVE_LNX_EAGAIN; /* would block → the core parks the caller */
	size_t n = g_mock_rlen < len ? g_mock_rlen : len;
	memcpy(buf, g_mock_rbuf, n);
	return (long)n;
}

static long mock_write(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p,
...
static long mock_ioctl(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p,
		       unsigned long cmd, unsigned long arg)
{
	...
	return -OVE_LNX_ENOTTY; /* unknown command */
}

static unsigned mock_poll(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o)
{
	(void)d;
	(void)o;
...
static const ove_lnx_file_t g_fs[] = {
	{.path = "/dev", .data = NULL, .size = 0, .mode = OVE_LNX_S_IFDIR | 0755u},
};
...
static void setup(ove_lnx_proc_t *p, ove_arena_t *arena)
{
	...
	ove_lnx_proc_set_rootfs(p, g_fs, 1);
	g_mock_rlen = g_mock_wlen = 0;
	g_mock_block = g_mock_released = g_mock_opened = 0;
	g_mock_val = 0;
	assert_int_equal(ove_lnx_dev_register(&mock_dev), 0); /* idempotent across tests */
}

static long dev_open(ove_lnx_proc_t *p, int flags)
{
	return ove_lnx_syscall(p, OVE_LNX_NR_openat, OVE_LNX_AT_FDCWD,
...
static void test_dev_open_close(void **state)
{
	...
	assert_int_equal(p.fds[fd].kind, OVE_LNX_FD_DEV);
	...
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_openat, OVE_LNX_AT_FDCWD,
					 (long)(uintptr_t) "/dev/nope", OVE_LNX_O_RDWR, 0, 0, 0),
			 -OVE_LNX_ENOENT);
...
static void test_dev_read_write(void **state)
{
	...
	long w = ove_lnx_syscall(&p, OVE_LNX_NR_write, fd, (long)(uintptr_t) "abcd", 4, 0, 0, 0);
	...
	g_mock_rlen = 2;
	char rb[8] = {0};
	long r = ove_lnx_syscall(&p, OVE_LNX_NR_read, fd, (long)(uintptr_t)rb, sizeof(rb), 0, 0, 0);
...
static void test_dev_ioctl(void **state)
{
	...
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_ioctl, fd, MOCK_IOC_SET,
					 (long)(uintptr_t)&v, 0, 0, 0),
			 0);
	uint32_t out = 0;
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_ioctl, fd, MOCK_IOC_GET,
					 (long)(uintptr_t)&out, 0, 0, 0),
			 0);
	assert_int_equal(out, 0xdeadbeef);

	/* An unknown command → -ENOTTY (not the console gate's blanket reject). */
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_ioctl, fd, 0x9999, 0, 0, 0, 0),
			 -OVE_LNX_ENOTTY);
...
static void test_dev_stat_lseek_poll(void **state)
{
	...
	assert_int_equal(st.st_mode & OVE_LNX_S_IFMT, OVE_LNX_S_IFCHR);
	assert_int_equal((int)st.st_rdev, (42 << 8) | 7);

	/* stat-by-path recognises the device node too. */
	memset(&st, 0, sizeof(st));
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_stat64, (long)(uintptr_t) "/dev/mock",
					 (long)(uintptr_t)&st, 0, 0, 0, 0),
			 0);
	assert_int_equal(st.st_mode & OVE_LNX_S_IFMT, OVE_LNX_S_IFCHR);

	/* lseek within the device's fixed extent (size 256). */
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_lseek, fd, 10, OVE_LNX_SEEK_SET, 0, 0, 0),
			 10);
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_lseek, fd, 0, OVE_LNX_SEEK_END, 0, 0, 0),
			 256);
	assert_int_equal(ove_lnx_syscall(&p, OVE_LNX_NR_lseek, fd, 300, OVE_LNX_SEEK_SET, 0, 0, 0),
			 -OVE_LNX_EINVAL); /* past the extent */

	/* poll reports the driver's readiness bits. */
	ove_lnx_pollfd pfd = {.fd = (int)fd, .events = OVE_LNX_POLLIN | OVE_LNX_POLLOUT};
	long pr = ove_lnx_syscall(&p, OVE_LNX_NR_poll, (long)(uintptr_t)&pfd, 1, 0, 0, 0, 0);
	assert_int_equal(pr, 1);
	assert_int_equal(pfd.revents & (OVE_LNX_POLLIN | OVE_LNX_POLLOUT),
...
static void test_dev_deferred_block(void **state)
{
	...
	assert_int_equal(ove_lnx_dev_retry(&p), -OVE_LNX_EAGAIN);

	/* Data arrives → the retry completes with the bytes (the run loop then resumes). */
	memcpy(g_mock_rbuf, "ok", 2);
	g_mock_rlen = 2;
	g_mock_block = 0;
	assert_int_equal(ove_lnx_dev_retry(&p), 2);
	assert_memory_equal(rb, "ok", 2);

	/* O_NONBLOCK does NOT park — it returns -EAGAIN straight to the program. */
	p.dev_wait = 0;
	long fd2 = dev_open(&p, OVE_LNX_O_RDWR | OVE_LNX_O_NONBLOCK);
	assert_true(fd2 >= 3);
	g_mock_block = 1;
	assert_int_equal(
		ove_lnx_syscall(&p, OVE_LNX_NR_read, fd2, (long)(uintptr_t)rb, sizeof(rb), 0, 0, 0),
		-OVE_LNX_EAGAIN);
	assert_int_equal(p.dev_wait, 0);

	ove_lnx_syscall(&p, OVE_LNX_NR_close, fd, 0, 0, 0, 0, 0);
	ove_lnx_syscall(&p, OVE_LNX_NR_close, fd2, 0, 0, 0, 0, 0);
}
...
static void test_dev_dup_refcount(void **state)
{
	...
	assert_int_equal(p.fds[fd2].kind, OVE_LNX_FD_DEV);
...
static void test_dev_getdents(void **state)
{
	...
	long dfd = ove_lnx_syscall(&p, OVE_LNX_NR_openat, OVE_LNX_AT_FDCWD,
				   (long)(uintptr_t) "/dev", OVE_LNX_O_RDONLY, 0, 0, 0);
	assert_true(dfd >= 3);

	uint8_t buf[256];
	long n = ove_lnx_syscall(&p, OVE_LNX_NR_getdents64, dfd, (long)(uintptr_t)buf, sizeof(buf),
				 0, 0, 0);
	assert_true(n > 0);
	...
	for (long off = 0; off < n;) {
		...
		if (strcmp(name, "mock") == 0) {
			found = 1;
			assert_int_equal(d_type, OVE_LNX_DT_CHR);
		}
