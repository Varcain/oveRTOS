#!/usr/bin/env python3
# Negative MPU-isolation test for the NuttX Linux personality (QEMU mps2-an500).
#
# Boots the firmware, logs in, and runs /usr/bin/segv — a tiny unprivileged Linux program that
# deliberately writes to kernel SRAM (0x20000000), which lies OUTSIDE the program's granted MPU
# regions (code RX + pool RW). NuttX runs the program UNPRIVILEGED behind that per-program MPU view
# (CONTROL.nPRIV + PRIVDEFENA), so the store raises a MemManage fault, which the personality must
# CONTAIN: the memfault handler kills segv like a default-action SIGSEGV (exit 139) and the shell
# survives. A broken isolation would instead let the store return (segv prints its "FAILED" line),
# fault the kernel (HardFault / up_assert / panic), or hang — all of which this test catches.
#
# The keystrokes are fed by a plain shell pipeline (printf ... | ove run > log), NOT a Python
# pty/subprocess reader: QEMU's mps2-an500 MPU emulation is timing-fragile, and any host-side reader
# (a pty's line discipline, a draining thread, even a Python parent's GIL/scheduler jitter) perturbs
