...
fn test_wait_all() {
    let eg = EventGroup::new().unwrap();
    eg.set_bits(BIT_0 | BIT_1);
    let actual = eg.wait_bits(BIT_0 | BIT_1, WaitFlags::WAIT_ALL, 100).unwrap();
...
fn test_wait_any() {
    let eg = EventGroup::new().unwrap();
    eg.set_bits(BIT_0);
    let actual = eg.wait_bits(BIT_0 | BIT_1, WaitFlags::NONE, 100).unwrap();
    assert!(actual & BIT_0 != 0);
}

fn test_wait_timeout() {
    let eg = EventGroup::new().unwrap();
    let result = eg.wait_bits(BIT_0, WaitFlags::WAIT_ALL, 10);
...
fn test_clear_on_exit() {
    let eg = EventGroup::new().unwrap();
    eg.set_bits(BIT_0 | BIT_1);
    let _actual = eg.wait_bits(BIT_0 | BIT_1, WaitFlags::WAIT_ALL | WaitFlags::CLEAR_ON_EXIT, 100).unwrap();
    let remaining = eg.get_bits();
    assert!(remaining & BIT_0 == 0);
    assert!(remaining & BIT_1 == 0);
}
...
fn test_cross_thread() {
    ...
    let actual = eg.wait_bits(BIT_0 | BIT_1, WaitFlags::WAIT_ALL, 500).unwrap();
    assert_eq!(actual & (BIT_0 | BIT_1), BIT_0 | BIT_1);
