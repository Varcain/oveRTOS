...
fn test_send_full_times_out() {
    ...
    q.send(&1, 0).unwrap();
    q.send(&2, 0).unwrap();
    let result = q.send(&3, 10);
    assert!(matches!(result, Err(Error::Timeout)));
}

fn test_receive_empty_times_out() {
    let q = Queue::<i32, 5>::new().unwrap();
    let result = q.receive(10);
    assert!(matches!(result, Err(Error::Timeout)));
}
...
fn test_struct_item() {
    ...
    q.send(&Pair { a: 10, b: 20 }, 0).unwrap();
    let out = q.receive(0).unwrap();
...
fn test_type_safety() {
    ...
    q8.send(&0xAB, 0).unwrap();
    let v8 = q8.receive(0).unwrap();
    assert_eq!(v8, 0xAB);

    let q32 = Queue::<u32, 4>::new().unwrap();
    q32.send(&0xDEADBEEF, 0).unwrap();
    let v32 = q32.receive(0).unwrap();
