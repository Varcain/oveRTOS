 ...
 *
 * Threads are heap-allocated as `std::unique_ptr<ove::Thread<...>>`
 * inside OVE_MAIN(), constructed with `std::make_unique` after
 * `pm::init()` has run.  Pair with apps/cpp/zeroheap/example_pm/ which
 * uses file-scope `ove::Thread<>` instances with embedded storage.
 */
