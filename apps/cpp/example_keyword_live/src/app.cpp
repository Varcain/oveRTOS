...
static void infer_thread(void *)
{
    ...
    for (;;) {
        ...
        if (read_count > AUDIO_SAMPLE_FREQ) read_count = AUDIO_SAMPLE_FREQ;
        ...
        if (peak < 10) { OVE_LOG_WRN("Audio silent"); continue; }

        // DC offset
        int64_t sum = 0;
        for (unsigned i = 0; i < read_count; i++) sum += audio_window[i];
        ...
        if (rc != OVE_OK) { OVE_LOG_ERR("Features failed: %d", rc); continue; }

        int prediction;
        float confidence;
        rc = classify_keyword(prediction, confidence);
        if (rc != OVE_OK) { OVE_LOG_ERR("Classify failed: %d", rc); continue; }
