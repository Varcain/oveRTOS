...
static void page_checkbox(lv::ObjectView p) {
	lv::Checkbox::create(p).text("Enable option").checked(true)
		.text_color(lv_color_white()).center();
}
...
static void page_chart(lv::ObjectView p) {
	...
	g_series = g_chart.add_series(
		lv_palette_main(LV_PALETTE_CYAN),
		LV_CHART_AXIS_PRIMARY_Y);
}

static void page_table(lv::ObjectView p) {
	auto table = lv::Table::create(p)
		.column_count(2)
		.row_count(4);
	...
	for (int32_t y = 0; y < 64; y++)
		for (int32_t x = 0; x < 64; x++)
			canvas.set_pixel(x, y,
				lv_color_make(uint8_t(x * 4), uint8_t(y * 4), 128));
	canvas.center();
}

static void page_calendar(lv::ObjectView p) {
	lv::Calendar::create(p)
		.size(240, 240)
		.today(2026, 4, 13)
		.showed(2026, 4)
		.center();
...
static void page_msgbox(lv::ObjectView p) {
	...
	btn.on_click(+[](lv_event_t *) {
		...
		mbox.add_title("Hello")
		    .add_text("Message box from the gallery.")
		    .add_close_button();
...
static const Page pages[] = {
	...
	{ "Roller",   page_roller   },
	{ "Spinbox",  page_spinbox  },
	{ "Text+Kbd", page_textarea },
	{ "Chart",    page_chart    },
	{ "Table",    page_table    },
	{ "List",     page_list     },
	{ "Image",    page_image    },
...
static constexpr int N_PAGES =
	static_cast<int>(sizeof(pages) / sizeof(pages[0]));
...
static void rebuild_page()
{
	...
	snprintf(buf, sizeof(buf), "%s (%d/%d)",
		 pages[g_page].name, g_page + 1, N_PAGES);
...
static void tick_cb(lv_timer_t *)
{
	...
	if (g_bar) g_bar.value(tick % 101, LV_ANIM_OFF);
	if (g_arc) g_arc.value(tick % 101);
	if (g_led && (tick % 10) == 0) g_led.toggle();
	if (g_chart && g_series) {
