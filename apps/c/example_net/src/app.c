...
static void test_netif_init(void)
{
	...
	if (ret != OVE_OK) { FAIL("netif_init", ret); return; }
	PASS("netif_init");
...
static void test_tcp(void)
{
	...
	int ret = ove_socket_open(&sock, &sock_storage,
				  OVE_AF_INET, OVE_SOCK_STREAM);
	if (ret != OVE_OK) { FAIL("socket_open TCP", ret); return; }
	...
	if (ret != OVE_OK) { FAIL("socket_connect", ret); ove_socket_close(sock); return; }
	PASS("socket_connect");
	...
	while (total < sizeof(buf) - 1) {
		ret = ove_socket_recv(sock, buf + total,
				      sizeof(buf) - 1 - total,
				      &received, 5000);
...
static void test_udp(void)
{
	...
	if (ret != OVE_OK) { FAIL("socket_open UDP", ret); return; }
	...
	if (ret != OVE_OK) { FAIL("socket_bind", ret); ove_socket_close(sock); return; }
	PASS("socket_bind");
	...
	size_t received = 0;
	ove_sockaddr_t src;
	ret = ove_socket_recvfrom(sock, buf, sizeof(buf) - 1,
				  &received, &src, 2000);
...
static void test_http(void)
{
	...
	if (ret != OVE_OK) { FAIL("http_client_init", ret); return; }
	...
	ret = ove_http_post(client, "http://httpbin.org/post",
			    "application/json",
			    json, strlen(json), &resp);
	if (ret == OVE_OK) {
		OVE_LOG_INF("  -> status %d, body %u bytes",
			    resp.status, (unsigned)resp.body_len);
		if (resp.status == 200) {
	...
	ret = ove_http_request_ex(client, OVE_HTTP_PUT,
				  "http://httpbin.org/put",
				  "application/json",
				  put_json, strlen(put_json),
				  headers, 2, &resp);
	if (ret == OVE_OK) {
		OVE_LOG_INF("  -> status %d, body %u bytes",
			    resp.status, (unsigned)resp.body_len);
...
static void test_mqtt(void)
{
	...
	if (ret != OVE_OK) { FAIL("mqtt_client_init", ret); return; }
	...
	ret = ove_mqtt_subscribe(client, "overtos/test",
				 OVE_MQTT_QOS0);
	if (ret == OVE_OK) {
		PASS("mqtt_subscribe");
	} else {
	...
	ret = ove_mqtt_publish(client, "overtos/test",
			       msg0, strlen(msg0), OVE_MQTT_QOS0);
	if (ret == OVE_OK) {
	...
	ret = ove_mqtt_publish(client, "overtos/test",
			       msg1, strlen(msg1), OVE_MQTT_QOS1);
	if (ret == OVE_OK) {
...
void ove_main(void)
{
	...
	OVE_LOG_INF("Networking example: init");
	...
	ret = ove_thread_create(&thread_handle, 8192, &desc);
	if (ret != OVE_OK) {
		OVE_LOG_ERR("Failed to create net thread: %d", ret);
		return;
	}
