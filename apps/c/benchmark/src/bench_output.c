...
void bench_print_header(const char *suite_name)
{
	...
	OVE_LOG("| %-27s | %-8s | %9s | %9s | %9s | %10s |\n",
		    "Case", "Type", "Min ns", "Avg ns", "Max ns", "Ops/s");
	OVE_LOG("%s\n", DIVIDER);
}
...
void bench_print_result(const bench_case_t *bc, const bench_result_t *result)
{
	...
	const char *type_str =
		bc->type == BENCH_TYPE_LATENCY ? "LATENCY" : "THROUGH";
	uint64_t avg_ns = 0;
	...
	OVE_LOG("| %-27s | %-8s | %9u | %9u | %9u | %10u |\n",
		    bc->name, type_str,
		    (unsigned int)result->min_ns,
		    (unsigned int)avg_ns,
		    (unsigned int)result->max_ns,
