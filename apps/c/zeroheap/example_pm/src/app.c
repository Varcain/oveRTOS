 ...
 *
 * Demonstrates the ove_pm_* API with file-scope static thread
 * allocation via OVE_THREAD_DEFINE_STATIC.  No `_create()` symbols are
 * linked; each thread's storage and stack live in BSS, initialised by
 * a __attribute__((constructor)) before ove_main() runs.
 */
...
OVE_THREAD_DEFINE_STATIC(sensor_th, 4096, sensor_thread, NULL, OVE_PRIO_NORMAL, "sensor");
OVE_THREAD_DEFINE_STATIC(monitor_th, 4096, monitor_thread, NULL, OVE_PRIO_LOW, "monitor");
