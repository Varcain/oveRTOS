...
static void multiple_rectangles_cb(void)
{
	...
	lv_obj_set_flex_align(lv_screen_active(), LV_FLEX_ALIGN_SPACE_EVENLY,
...
static void multiple_labels_cb(void)
{
	...
	lv_text_get_size(&s, "Hello LVGL!",
			 lv_obj_get_style_text_font(lv_screen_active(), 0),
...
static void widgets_demo_cb(void)
{
	...
	/* Gauge 1: circular 360° with 3 concentric arcs */
	{
		...
		static const struct { uint32_t t1; uint32_t t2; lv_palette_t pal; int32_t margin; } arcs[] = {
		...
		for (int i = 0; i < 3; i++) {
			...
			lv_obj_set_size(arc, 180 - arcs[i].margin * 2,
					     180 - arcs[i].margin * 2);
			lv_obj_center(arc);
				    ...
				    lv_subject_t *subject)
{
	...
	lv_label_set_text_fmt(label,
			      ...
			      scene_name,
			      (unsigned)m.fps, (unsigned)m.cpu,
			      (unsigned)(m.render_avg_time + m.flush_avg_time),
			      (unsigned)m.render_avg_time,
			      (unsigned)m.flush_avg_time);
...
static void summary_create(void)
{
	...
	lv_obj_set_style_text_color(table,
				    lv_palette_darken(LV_PALETTE_BLUE_GREY, 2),
				    LV_PART_ITEMS);
	lv_obj_set_style_border_color(table,
				      lv_palette_darken(LV_PALETTE_BLUE_GREY, 2),
				      LV_PART_ITEMS);
	lv_obj_add_event_cb(table, table_draw_task_event_cb,
	...
	for (uint32_t i = 0; scenes[i].create_cb; i++) {
		...
		if (scenes[i].measurement_cnt <= 1) {
			...
			lv_table_set_cell_value_fmt(table, i + 2, 3, "%u ms (%u + %u)",
						    (unsigned)(render + flush),
						    (unsigned)render,
						    (unsigned)flush);

			OVE_LOG_INF("%s, %u%%, %u, %u, %u, %u",
				    scenes[i].name, (unsigned)cpu,
				    (unsigned)fps, (unsigned)(render + flush),
				    (unsigned)render, (unsigned)flush);
	...
	if (valid < 1) {
		...
		OVE_LOG_INF("All avg, %u%%, %u, %u, %u, %u",
			    (unsigned)avg_cpu, (unsigned)avg_fps,
			    (unsigned)(avg_render + avg_flush),
...
void ove_main(void)
{
	...
	/* Graphics thread */
	{
		...
		if (ret != OVE_OK) {
			OVE_LOG_ERR("Failed to create graphics thread: %d", ret);
			return;
		}
	}
