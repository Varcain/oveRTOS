...
fn appMain() void {
    ove.log.inf("pm example (zero-heap mode): init", .{});
    ...
    ove.log.inf("pm example (zero-heap mode): ready (battery={d}%)", .{battery_pct.load(.monotonic)});

    ove.run();

    ove.log.inf("pm example (zero-heap mode): shutdown", .{});
}
