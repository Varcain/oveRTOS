...
fn generateFeatures(
    ...
    _ = storage;
    var preproc = try ove.Model.create(&cfg);
...
fn classifyKeyword(
    ...
    _ = storage;
    var classifier = try ove.Model.create(&cfg);
...
fn appMain() void {
    ...
    infer_thread = ove.Thread(8192).create("infer", inferThread, ove.thread.prio.normal) catch {
