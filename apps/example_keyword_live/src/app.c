...
/* Snapshot of first DMA callback's raw input for debugging */
static int16_t dma_snapshot[64];
static volatile int dma_snapshot_taken;
 ...
 *   3. Captures a DMA snapshot for diagnostics
 */
			     ...
			     struct ove_audio_buf *out)
{
	...
	/* Capture one raw DMA buffer snapshot for diagnostics */
	if (!dma_snapshot_taken && frames >= 64) {
		memcpy(dma_snapshot, src, 64 * sizeof(int16_t));
		dma_snapshot_taken = 1;
	}
...
/*
 * Compute RMS and peak amplitude of an audio buffer for diagnostics.
 */
static void audio_stats(const int16_t *buf, unsigned int count,
			int16_t *peak_out, unsigned int *rms_out)
{
	int32_t peak = 0;
	uint64_t sum_sq = 0;
	...
	*peak_out = (int16_t)peak;
	/* Integer RMS approximation */
	if (count > 0)
		*rms_out = (unsigned int)(sum_sq / count);
	else
		*rms_out = 0;
}
...
static void infer_thread(void *arg)
{
	...
	for (;;) {
		...
		int16_t peak;
		unsigned int rms;
		audio_stats(audio_window, read_count, &peak, &rms);
...
static ove_thread_t infer_thread_handle;
