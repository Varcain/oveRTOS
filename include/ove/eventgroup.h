 ...
 * - **Static storage** via @ref ove_eventgroup_init / @ref ove_eventgroup_deinit.
 * - **Heap** via @ref ove_eventgroup_create / @ref ove_eventgroup_destroy —
 *   available only when @c OVE_HEAP_EVENTGROUP is defined.
 *
...
#define ove_eventgroup_create(...) \
	_Static_assert(0, "ove_eventgroup_create() requires heap. Use ove_eventgroup_init() in zero-heap mode.")
#define ove_eventgroup_destroy(...) \
	_Static_assert(0, "ove_eventgroup_destroy() requires heap. Use ove_eventgroup_deinit() in zero-heap mode.")
