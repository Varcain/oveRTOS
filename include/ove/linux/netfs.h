 ...
 * run-loop coordinator pumps the transport each pass via ove_lnx_netfs_retry and
...
#define OVE_LNX_NETFSW_OPEN 1u	  /**< open: Twalk -> Tlgetattr -> Tlopen -> install fd. */
#define OVE_LNX_NETFSW_READ 2u	  /**< read: Tread -> copy to guest. */
#define OVE_LNX_NETFSW_GETDENTS 3u /**< getdents64: Treaddir -> emit dirent64 records. */
#define OVE_LNX_NETFSW_STAT 4u	  /**< path stat: Twalk -> Tlgetattr -> Tclunk -> fill guest stat. */
#define OVE_LNX_NETFSW_EXECFETCH 5u /**< Phase B: read a whole remote ELF into the exec staging buffer. */

/* ---- boot: mount config + connection init (coordinator thread) ------------- */

/**
 * @brief Configure the static mount (call at app boot, before ove_lnx_run).
 ...
 */
void ove_lnx_netfs_mount_config(const char *mountpoint, const uint8_t ip[4], uint16_t port,
				const char *aname, const char *uname);

/** @brief Open the socket + do the blocking Tversion/Tattach handshake. Coordinator
 *  thread only (from ove_lnx_dev_autoreg_all's region). A down server is non-fatal:
 ...
 *  completion the retry marshals the attrs into the guest buffer via ove_lnx_netfs_fill_stat.
 ...
 *  in-flight request, and return the completed Linux-ABI result or -OVE_LNX_EAGAIN. */
