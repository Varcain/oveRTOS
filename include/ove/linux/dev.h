 ...
 * register an @ref ove_lnx_dev (a path like "/dev/fb0" + an ops vtable) and the
 ...
 * block returns @c -OVE_LNX_EAGAIN; the core parks the calling process
 * (@c dev_wait) and the run-loop coordinator retries the op on its own thread
 * — the same park/retry pattern the pipe layer uses (see ove_lnx_pipe_retry).
 *
 ...
 * negated @c OVE_LNX_E* on failure. A blocking read/write/ioctl returns
 * @c -OVE_LNX_EAGAIN to have the core park + retry the caller. Any user pointer
 * a handler dereferences must first pass @c user_ok (handlers run PRIVILEGED).
 */
struct ove_lnx_dev_ops {
	/** Open: initialise @p o for this open (optional). */
	long (*open)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, int flags);
	/** Release: last close of this open (optional). */
	long (*release)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o);
	/** read(2): bytes read, 0 (EOF), or -EAGAIN to block. @p buf is pre-@c user_ok'd. */
	long (*read)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p, void *buf,
		     size_t len);
	/** write(2): bytes written, or -EAGAIN to block. @p buf is pre-@c user_ok'd. */
	long (*write)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p,
		      const void *buf, size_t len);
	/** ioctl(2): every user deref of @p arg must pass @c user_ok. -ENOTTY = unknown cmd. */
	long (*ioctl)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p,
		      unsigned long cmd, unsigned long arg);
	/** poll(2): POLLIN|POLLOUT bits currently ready; must never block. */
	unsigned (*poll)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o);
	...
	long (*mmap)(struct ove_lnx_dev *d, struct ove_lnx_dev_open *o, ove_lnx_proc_t *p, size_t len,
...
struct ove_lnx_dev {
	...
	const struct ove_lnx_dev_ops *ops;   /**< Class operations. */
...
#define OVE_LNX_MAP_NC 0u  /**< Normal, non-cacheable. */
 ...
 *
 * Call before @c ove_lnx_run (or from @c ove_lnx_dev_autoreg_all). @p dev is
