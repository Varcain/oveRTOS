...
| `__WFI()` (sleep until interrupt) | `ove_pm_state_set(OVE_PM_IDLE)` |
| busy-loop on a flag | wait on a semaphore or event group |
...
```c
/* Option A: signal a semaphore */
void button_isr(ove_gpio_t pin, void *_) {
    ove_semaphore_give_isr(button_sem);
}

/* Option B: post to a work queue */
void button_isr(ove_gpio_t pin, void *_) {
    ove_work_submit(&button_work);
}
```
...
The ISR registration goes through `ove_gpio_set_callback(pin, isr, arg)`; the underlying NVIC wiring is done by the backend.
...
|---|---|
| `HAL_UART_Init(&huart1)` | `ove_uart_open(&uart, OVE_UART_DEV_0, &cfg)` |
| `HAL_UART_Transmit(&huart1, b, n, T)` | `ove_uart_write(uart, b, n, T_ms)` |
| `HAL_UART_Receive(&huart1, b, n, T)` | `ove_uart_read(uart, b, n, T_ms)` |
| `HAL_I2C_Mem_Read(&hi2c1, addr<<1, reg, 1, b, n, T)` | `ove_i2c_mem_read(i2c, addr, reg, 1, b, n)` |
| `HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET)` | `ove_gpio_write(led, 1)` |

The big wins:

- **Identical code across STM32, NXP, nRF, RP2040** — only the board file changes.
- **Timeout in ms, not ticks** — no `pdMS_TO_TICKS` ceremony.
...
- **No more `volatile` discipline for cross-thread communication** — use proper synchronization primitives. The compiler may now reorder operations across `if (flag)` checks. Replace with `ove_eventgroup_*` or `atomic_*`.
