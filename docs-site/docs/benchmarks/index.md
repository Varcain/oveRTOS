...
The zero-heap reports are generated by flashing the `_zh` benchmark
apps and running `scripts/bench_compare.py --page-mode zeroheap`
against the captured serial logs:

```bash
make stm32f746.freertos.benchmark_zh           # C
make stm32f746.freertos.benchmark_cpp_zh       # C++
make stm32f746.freertos.benchmark_rust_zh      # Rust
make stm32f746.freertos.benchmark_zig_zh       # Zig
```
