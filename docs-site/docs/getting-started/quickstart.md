...
This page takes you from a fresh checkout to a running oveRTOS application on the host. No hardware, no cross-compilation — just `make`, your existing C compiler, and a few minutes.
...
Cross-compilation, QEMU, Rust and Zig are **not** needed for this quickstart. They are required only when you target embedded RTOS backends — see [Setup](setup.md) for the full matrix.
...
| 4 | `make host.posix.example_c` | Loads the `host.posix.example_c` defconfig fragment (POSIX backend, C example app) into `.config` |
...
|---|---|
| C example on POSIX host (this quickstart) | `make host.posix.example_c` |
| C example on QEMU running FreeRTOS | `make qemu.freertos.example_c` |
| Rust example on QEMU running NuttX | `make qemu.nuttx.example_rust` |
| C++ example on STM32F7 Discovery + Zephyr | `make stm32f746.zephyr.example_cpp` |
| Zero-heap C example on POSIX host | `make host.posix.example_c_zh` |
...
| Create your own app outside the oveRTOS source tree | [External Apps](../build-system/external-apps.md) — or just run `ove app new --lang c --name my_app` |
...
make host.posix.example_c && make && make run
```
